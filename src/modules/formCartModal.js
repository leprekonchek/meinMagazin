...
let _emptyInitCart = () => {
    ...
    $(`<div class="description">But it is never too late to change this...</div>`).appendTo(".modal-desc");
...
let _fillCartModal = ({
    ...
    if (prod.special_price !== null) {
        $price = parseFloat(prod.special_price) * parseInt(quantity);
    } else {
        $price = parseFloat(prod.price) * parseInt(quantity);
    }
...
module.exports = {_emptyInitCart, _fillCartModal, _initForm, _sucessOrder, $totalSum};